...
const customValidators = {
  ...
  nigerianPhone: (value) => {
    if (!value) return; // Allow optional fields
    const phoneRegex = /^(\+234|0)[789][01]\d{8}$/;
    if (!phoneRegex.test(value)) {
      throw new Error('Must be a valid Nigerian phone number (+234xxxxxxxxxx or 0xxxxxxxxxx)');
    }
  },
  ...
  studentId: (value) => {
    ...
    if (!studentIdRegex.test(value)) {
      throw new Error('Student ID must be 3-20 alphanumeric characters');
    }
  },
...
// Extend Joi with custom validators - using external() method instead
let extendedJoi = Joi;
...
const validationSchemas = {
  ...
  register: Joi.object({
    ...
    password: Joi.string().min(6).required().external(customValidators.strongPassword),
  ...
  changePassword: Joi.object({
    ...
    newPassword: Joi.string().min(6).required().external(customValidators.strongPassword),
  ...
  createStudent: Joi.object({
    ...
    studentId: Joi.string().min(3).max(20).pattern(/^[A-Za-z0-9]{3,20}$/).required().external(customValidators.studentId),
    ...
    phone: Joi.string().optional().external(customValidators.nigerianPhone)
  }),
  ...
  updateStudent: Joi.object({
    ...
    phone: Joi.string().optional().allow('').external(customValidators.nigerianPhone)
  }),
...
const validateStudentId = (studentId) => {
  const { error } = Joi.string().min(3).max(20).pattern(/^[A-Za-z0-9]{3,20}$/).validate(studentId);
  return !error;
};
